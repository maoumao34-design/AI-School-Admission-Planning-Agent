...
- **数据（双类 3666 卡）**：物理 2511 / 996 校 + 历史 1155 / 803 校，jseea 2025 投档线 PDF 批量解析转卡。
...
- **最低位次 `min_rank`**：3666 卡中仅 61 条手填有值，3621 条 null 标「待一分一段表派生」（一分一段表为图片、工具链暂无 OCR）；引擎走投档分差兜底（MAO-26），位次后续补精。
...
- **响应式布局**：Tailwind 自适应布局覆盖桌面与移动端基础浏览要求。
...
2. 框架自动识别为 **Next.js**；构建命令已由 [`vercel.json`](./vercel.json) 固定为 `prisma generate && next build`（显式生成 Prisma Client 后构建，保证部署流程稳定）。
